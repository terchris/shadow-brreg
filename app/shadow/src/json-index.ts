 ...
 * 1. Get the date of the last update from the urbalurba_status_json table
 * 2. Get the changes from brreg API since the last update
 * 3. Store the changes from the API in the oppdaterteEnheter_json table
 * 4. for all the changes in the oppdaterteEnheter table: update the shadowbrreg_json table (delete, add or update)
 * 
 */
...
async function testDatabaseConnection(): Promise<boolean> {
    ...
    const firstOrganizationsQuery = `SELECT * FROM shadowbrreg_json`;
...
async function getLastDateWeStoredOpdatesFromBrregAPI(): Promise<string> {
    try {
        const res = await pool.query(`SELECT database_download_date, last_brreg_update_date FROM urbalurba_status_json LIMIT 1`);
